...
RUN set -eux; \
    TFLINT_URL="https://github.com/terraform-linters/tflint/releases/latest/download/tflint_linux_arm64.zip"; \
...
RUN set -eux; \
    OPA_URL="https://github.com/open-policy-agent/opa/releases/latest/download/opa_linux_arm64"; \
...
ENV ANSIBLE_RETRY_FILES_ENABLED=False \
    ...
    PYTHONUNBUFFERED=1 \
    TF_PLUGIN_CACHE_DIR=/usr/local/terraform.d/plugin-cache \
    TF_IN_AUTOMATION=1 \
    TF_INPUT=0 \
    PAGER=cat \
...
RUN set -eux; \
    for i in {1..5}; do curl -LsS "https://awscli.amazonaws.com/awscli-exe-linux-aarch64.zip" -o /tmp/awscli.zip && break || sleep 15; done ;\
